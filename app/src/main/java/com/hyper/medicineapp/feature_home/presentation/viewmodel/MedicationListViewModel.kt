...
class MedicationListViewModel @Inject constructor
    (private val medicationRepository: MedicationRepository) : ViewModel() {
    ...
    fun getMedicationList() {
        viewModelScope.launch {
            _uiState.value = MedicationListState(isLoading = true)

            medicationRepository.getMedicationList().let { medicationList ->
                when (medicationList) {
